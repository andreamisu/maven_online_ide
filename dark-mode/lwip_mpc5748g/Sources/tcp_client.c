...
static void create_tcp_connection() {
	...
	// TODO: implement
	PINS_DRV_ClearPinExIntFlag(11);   // NEW

	PINS_DRV_TogglePins(PTA, 0x1u << 4u);

	ip_addr_t remote_ip;
	ip_addr_t local_ip;

	uint16_t remote_port = 8080;
	uint16_t local_port = 0; // will find available port automatically

	ipaddr_aton("35.242.166.217", &remote_ip);   //  my win: 192.168.178.47     my mac: 192.168.178.39   ,  testserver: 35.242.166.217
...
void start_tcp_client() {
	...
	while (1) {
		...
		if (counter % 10000 == 0 && netif.ip_addr.u_addr.ip4.addr) {     // 0 removed
			  PINS_DRV_WritePin(PTA, 4, 0x1u);   // 0x1u means off    // NEW
			  INT_SYS_InstallHandler(244, &create_tcp_connection, NULL);     // NEW
			  INT_SYS_EnableIRQ(244);    // NEW
